...
rom_start :
{
 ...
 service_area : { _service_list_start = .; KEEP(*(SORT(._service.static.*_?_*))); KEEP(*(SORT(._service.static.*_??_*))); KEEP(*(SORT(._service.static.*_???_*))); KEEP(*(SORT(._service.static.*_????_*))); KEEP(*(SORT(._service.static.*_?????_*))); _service_list_end = .;; }
 initlevel_error :
 {
  KEEP(*(SORT(.z_init_*)))
 }
